// app/payments/token-fail/page.tsx
import Link from "next/link";
import type { JSX } from "react";

const FAIL_MESSAGES: Record<string, string> = {
	invalid_params: "결제 정보가 올바르지 않아요.",
	purchase_not_found: "구매 정보를 찾을 수 없어요.",
	amount_mismatch: "결제 금액이 일치하지 않아요.",
	config: "결제 서버 설정이 아직 완료되지 않았어요. 관리자에게 문의해주세요.",
	confirm_failed: "결제 승인에 실패했어요. 카드사 승인이 거절됐을 수 있어요.",
	record_failed: "결제는 승인됐지만 적립 중 문제가 생겼어요. 고객센터로 문의해주세요.",
};

type TokenPaymentFailPageProps = {
	searchParams: Promise<{ code?: string; message?: string }>;
};

export default async function TokenPaymentFailPage({
	searchParams,
}: TokenPaymentFailPageProps): Promise<JSX.Element> {
	const { code, message } = await searchParams;
	const description =
		(code && FAIL_MESSAGES[code]) || message || "충전이 완료되지 않았어요. 다시 시도해주세요.";
	...
	return (
		<main className="mx-auto flex min-h-screen w-full max-w-md flex-col items-center justify-center gap-5 px-5">
			...
			<div className="flex flex-col items-center gap-1.5">
				<h1 className="text-xl font-bold">충전에 실패했어요</h1>
